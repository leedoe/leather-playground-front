...
class TanneryDetail extends React.Component {
  state = {
    tannery: {}
  }

  getTanneryDetail = () => {
    Axios.get(`${process.env.REACT_APP_SERVERURL}/api/tannerys/${this.props.match.params.pk}`).then((response) => {
      this.setState({tannery: response.data,})
    }).catch((e) => {
  ...
  render () {
    ...
    return (
      <div className={classes.mainContainer}>
        ...
        <Comments type={`tannery`} fk={this.props.match.params.pk}/>
      </div>
    )
  }
}
...
export default connect(mapStateToProps)(withStyles(useStyles, { withTheme: true })(withRouter(withSnackbar(TanneryDetail))));